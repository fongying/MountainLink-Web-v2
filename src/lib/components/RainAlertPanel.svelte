<script lang="ts">
  ...
  export let title = '豪雨警報';

  let alerts: RainAlert[] = [];
  let updatedAt: number | null = null;
  let loading = true;
  let error = '';

  function buildUrl() {
    const url = new URL('/api/alerts/rain', window.location.origin);
  ...
  async function loadAlerts() {
    try {
      ...
      const json = (await res.json()) as { updatedAt: number; alerts: RainAlert[] };
      alerts = json.alerts ?? [];
...
<section class="card rainCard">
  <div class="cardHeader">
    <div>
      <h2>{title}</h2>
      <p class="muted">中央氣象署 W-C0033-003 即時特報</p>
    </div>
    <div class="meta">
      {#if updatedAt}
    ...
    <div class="alerts">
      {#each alerts as a (a.id)}
        <article class="alert">
          ...
          <div class="areas">
            {#each a.areas as area}
              <span class="areaChip">{area}</span>
            {/each}
          </div>
        </article>
      {/each}
